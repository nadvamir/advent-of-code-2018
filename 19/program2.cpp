...
Register execute(const std::vector<Command>& cmds, Register init) {
    while (init[IP] < cmds.size() && init[IP] != 34) {
...
int sum_of_divisors(int target) {
    int sum = 0;
...
void run() {
    ...
    Register init{1, 0, 0, 0, 0, 0};
    auto target = execute(commands, init)[2];
    auto answer = sum_of_divisors(target);
